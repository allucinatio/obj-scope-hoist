...
var y is not a global variable, it's local to function scopeThis() which is local to funtion().

console.log(y) would return an error because y is defined out of scope inside of scopeThis().

console.log(x) would return "I'm a local variable" because var x is declared above function scopeThis() that calls
...
console.log(x) would return an error because x has not yet been defined/

console.log(y) would return an error because there is no var y in this example. intentional? Newline error? we may never know...
...
// told to skip this one because of a bug
...
console.log(add) inside the IFEE would print 4, because add is defined within scope

console.log(add) outside the IFEE would return an error because add is defined inside the IFEE, outside of scope.
